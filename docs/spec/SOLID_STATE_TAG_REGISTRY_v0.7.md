...
# v0.7 additions — Phase 1.1 resolution patch

## Occupational / controlled-hardening flags

| Flag | Meaning |
|---|---|
| `ROUTE_CAR_MATERIAL_EXPOSURE` | Active multi-year occupational material exposure sequence |
| `ROUTE_CAR_MATERIAL_EXPOSURE_HISTORY` | Occupational exposure sequence completed |
| `ROUTE_MED_CONTROLLED_HARDENING` | Controlled-hardening medical course is committed |
| `ROUTE_MED_CONTROLLED_HARDENING_STAGE2` | Controlled-hardening course reached its second mandatory stage |

## Anomalous route flags

| Flag | Meaning |
|---|---|
| `ROUTE_ANO_OBJECT_CONTINUITY` | T1028 continuity-through-alteration route is committed |
| `ROUTE_ANO_SOUL_ESCAPE` | T1029 displaced-self route is committed |
| `SPC_REIN_ECHO` | A prior-life/reincarnation echo has occurred |

## Late-life route flags

| Flag | Meaning |
|---|---|
| `ROUTE_MED_LATE_CONTINUITY` | 65+ longevity/continuity preservation route active |
| `ROUTE_FAM_LATE_LEGACY` | 65+ family legacy placement route active |

## Structured tag registries

Event `routeTags` are now canonically defined by:

`content/registries/SOLID_STATE_ROUTE_TAG_REGISTRY_v1.0.json`

The loader/validator must reject unknown route tags.

Species material refinement hooks are now canonically defined inside:

`content/registries/SOLID_STATE_SPECIES_REGISTRY_v1.2.json`

Optional event `refinementTags` must be registered there. Refinement tags are local event/variant modifiers and do not add another drafting layer.
