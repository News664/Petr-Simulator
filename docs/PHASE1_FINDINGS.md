...
| 2 | Endings are effectively unreachable: 0.08–0.77% completion (6.1% with T1027) | **blocking** |
| 3 | Fallback share at age 25+ is 34–35% against a 2% target | high |
| 4 | 95–99% of route schedules expire unfired | high |
| 5 | Ending coverage is 1–13 of 24; two endings are unreachable by construction | medium |
| 6 | Material determinism is far *below* its guardrails, not above | informational |
| 7 | First manifestation is 53% WOOD and pinned to age 18 | medium |
...
|---|---:|---|
| no-talents | 1 685 / 2 000 (84.3%) | 4, 5 |
| uniform-three | 1 659 / 2 000 (83.0%) | 4, 5 |
| threshold-talents | 1 677 / 2 000 (83.9%) | 4, 5 |
| material-talents | 1 654 / 2 000 (82.7%) | 4, 5 |
| anomalous-talents | 1 706 / 2 000 (85.3%) | 4, 5 |
...
|---|---:|---:|---:|---:|
| no-talents | **0.11%** | 99.9% | 15.6% | 26 / 49 / 52 |
| uniform-three | **0.77%** | 99.2% | 22.1% | 27 / 51 / 61 |
| threshold-talents | **0.08%** | 99.9% | 15.8% | 26 / 49 / 52 |
| material-talents | **0.23%** | 99.8% | 15.7% | 26 / 50 / 53 |
| anomalous-talents | **6.12%** | 93.9% | 80.7% | 44 / 55 / 62 |
...
holding T1027, whose provisional +15 start FIX raises completion **56×** and
...
- `targetEndingAgeShare` is missed in **every band in every scenario**. Observed
  ...
  18–22%.
- Median ending age is 41–46 against a target distribution centred on 25–34.
...
| Fallback share of all event-years | 26.9–27.9% | — |
...
| Route climax rate (run fires a climax schedule) | 0.77% |
| Route abandonment/expiry rate (run has ≥1 expired schedule) | 98.6% |
| Expired schedules, total | 30 382 across 10 000 runs |
| Priority collision rate | 1.1% |
...
Priority collisions are rare (1.1% of runs), so the priority-class design is not
...
| anomalous-talents | 13 / 24 |

`END-MED-001` "Last Version of You" dominates every scenario (79% of completions
in anomalous, 74% in uniform-three) because `EVT-INS-MED-0003` gates at
...
|---|---|---|
| P(final X \| hint X) | 1.7–4.8% (18.3% max in anomalous) | ≤ 70% |
| P(final X \| first manifestation X) | 1.5–4.1% (17.9% max in anomalous) | ≤ 80% |
| Multiple manifestations before commitment | 99.0–99.2% | should be common |
| Final-material entropy by species | 1.32–1.49 bits | — |
...
Species differentiation is present but mild (entropy 1.32 for WINGED_KIN vs 1.49
for DEMONKIN), which is consistent with soft tendencies that never hard-lock.

Average ages: first hint 9.4, first manifestation 18.3–18.5, commitment
33.5–35.9.
...
manifestation in ~53% of runs (5 300 of 10 000) while the other six families
...
|---|---|---:|---:|
| T1002 Knows Excel | `INT>=5` | 99.9% | 4.4 |
| T1007 Pretty Privilege | `CHR>=8` | 90.2% | 15.1 |
| T1009 Frugal | `MNY<=3` | 89.5% | 6.7 |
| T1017 Late Bloomer | `AGE>=35 & CHR<=5` | 76.0% | 91.7 |
...
age of **91.7**, far past any intended life span, because `CHR<=5` is usually
only satisfied after decades of CHR drift. In a run that ended at a target age
of 25–44 it would rarely fire at all — its 76% activation rate is an artefact of
