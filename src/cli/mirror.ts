...
function registryMirrors(root: string): RegistryMirror[] {
  ...
  const routeTagJson = path.join(registries, 'SOLID_STATE_ROUTE_TAG_REGISTRY_v1.3.json');
  const talentCsv = path.join(registries, 'SOLID_STATE_TALENT_REGISTRY_v1.2.csv');
  return [
    {
      source: routeTagJson,
      markdown: path.join(registries, 'SOLID_STATE_ROUTE_TAG_REGISTRY_v1.3.md'),
      render: () =>
        renderRouteTagRegistryMarkdown(
