...
import type { FactionSummary, SanityPlan } from './phase1_3.js';
...
function factionSection(content: ContentBundle, metrics: MetricsSummary, seeds: FactionSeedIncidence): string[] {
  ...
  out.push(
    table(
      ['Faction', 'Route tag', 'Contact rate', 'Mean first-contact age', 'Climax rate', 'Endings'],
...
// ---------------------------------------------------------------------------
// Phase 1.3 sanity report
// ---------------------------------------------------------------------------

export interface Phase13Payload {
  generatedAt: string;
  contentVersion: string;
  balanceVersion: string;
  planVersion: string;
...
/**
 * Renders the Phase 1.3 sanity diagnostic.
 *
 * The H2A gate is decided on correctness, so the correctness counters lead and
 * the balance numbers follow as warnings. Nothing here selects a threshold,
 * switches a baseline or proposes content.
 */
export function renderPhase13Report(content: ContentBundle, payload: Phase13Payload): string {
  const out: string[] = [];
  const m = payload.metrics;
  const f = payload.factions;
  const factionNames = Object.keys(f.byFaction).sort();

  out.push('# SOLID STATE — Phase 1.3 Sanity Diagnostic');
  out.push('');
  out.push('**Measurements only.** Generated by the headless engine from');
  out.push('`content/balance/SOLID_STATE_PHASE1_3_SANITY_PLAN_v0.1.json`.');
  out.push('');
  out.push(payload.planPurpose);
  out.push('');
  out.push(
    'Narrative analysis lives in [`docs/PHASE1_3_FINDINGS.md`](../docs/PHASE1_3_FINDINGS.md) and decisions in ' +
      '[`docs/OPEN_QUESTIONS.md`](../docs/OPEN_QUESTIONS.md). This file is regenerated on every run — do not ' +
      'record decisions here.',
  );
  out.push('');
  out.push(
    table(
      ...
      [
        ...
        ['Balance constants', `v${payload.balanceVersion}`],
        ['Sanity plan', `v${payload.planVersion} (${payload.planStatus})`],
        ['Base seed', `\`${payload.baseSeed}\``],
        ['Runs', String(payload.settings.runs)],
        ['Species sampling', payload.settings.speciesMode],
        ['Talent scenario', payload.settings.talentScenario],
        ['Allocation policy', payload.settings.allocationPolicy],
        ['Family weighting', `\`${payload.settings.familyWeightMode}\``],
        ['FIX thresholds', payload.settings.thresholdMode],
        ['Pre-25 coverage policy', payload.settings.pre25CoveragePolicy],
      ],
    ),
  );
  out.push('');
  ...
  // --- Correctness first: this is what the H2A gate reads. ------------------
  out.push('## 1. Correctness counters');
  out.push('');
  out.push('These decide the H2A gate. Balance misses below are warnings, not blockers.');
  out.push('');
  out.push(
    table(
      ['Counter', 'Value', 'Required'],
      [
        ['Illegal faction lifecycle transitions', String(f.illegalTransitionCount), '0'],
        ['Runs holding two lifecycle states for one faction', String(f.illegalLifecycleStateCount), '0'],
        ['Personalized faction events after OPTED_OUT/CLOSED', String(f.personalEventsAfterExitCount), '0'],
        ['Pre-25 content coverage defects', String(m.coverageErrors), '0'],
        ['Pre-25 generic fallback events', String(m.pre25FallbackYears), '0'],
      ],
    ),
  );
  out.push('');
  ...
  // --- Completion -----------------------------------------------------------
  out.push('## 2. Completion and ending age');
  out.push('');
  out.push(
    table(
      ['Outcome', 'Runs', 'Share'],
  ...
  out.push('');
  out.push(
    `Average run length **${num(m.averageRunLengthYears)} years**; ending age mean **${num(m.averageEndingAge)}**, ` +
      `median **${num(m.medianEndingAge)}**, p10 **${num(m.endingAgeP10, 0)}**, p90 **${num(m.endingAgeP90, 0)}**. ` +
      `Endings at 65+: **${pct(m.endingShare65Plus)}**.`,
  );
  out.push('');
  out.push(endingBucketTable(m));
  ...
  // --- Faction lifecycle ----------------------------------------------------
  out.push('## 3. Faction lifecycle');
  out.push('');
  out.push(
    `Runs with at least one faction contact: **${pct(f.anyContactRate)}**. ` +
      `Faction-caused endings: **${f.factionEndingRuns}** of ${m.completed} completed runs ` +
      `(**${pct(f.factionEndingShare)}**).`,
  );
  out.push('');
  out.push('Incidence by faction, as a share of all runs. A run is counted once per state it ever reached.');
  out.push('');
  out.push(
    table(
      ...
      factionNames.map((name) => {
        ...
        return [
          name,
          pct(stats.newsRate),
          pct(stats.contactRate),
          pct(stats.everReachedRate['CONTACTED']),
          pct(stats.everReachedRate['ENGAGED']),
          pct(stats.everReachedRate['COMMITTED']),
          pct(stats.everReachedRate['OPTED_OUT']),
          pct(stats.everReachedRate['CLOSED']),
          pct(stats.targetedRate),
  ...
  out.push('');
  out.push('Applied transitions by target state:');
  out.push('');
  out.push(
    table(
      ['Target state', 'Transitions'],
      sortedEntries(f.transitionCounts).map(([state, count]) => [`\`${state}\``, String(count)]),
    ),
  );
  out.push('');
  out.push('Years from CONTACTED to each later milestone (mean / median over runs that reached it):');
  out.push('');
  const milestones = ['ENGAGED', 'COMMITTED', 'OPTED_OUT', 'CLOSED', 'ENDING'];
  out.push(
    table(
      ['Faction', ...milestones],
      factionNames.map((name) => {
        ...
        return [
          ...
          ...milestones.map((milestone) => {
            const entry = stats.yearsFromContact[milestone];
            if (!entry || entry.runs === 0) return '—';
            return `${num(entry.mean, 1)} / ${num(entry.median, 1)} (n=${entry.runs})`;
          }),
        ];
      }),
    ),
  ...
  out.push('');
  out.push('How faction endings arrived:');
  out.push('');
  out.push(
    table(
      ...
      [
        [
          'Committed ladder (CONTACT → DISPOSITION → ESCALATION → CLIMAX)',
          String(f.committedLadderFactionEndings),
          pct(f.committedLadderShareOfFactionEndings),
        ],
        [
          'Sudden (strongly conditioned disposition/escalation variant)',
          String(f.suddenFactionEndings),
          pct(f.suddenShareOfFactionEndings),
        ],
      ],
    ),
  );
  ...
  // --- Endings --------------------------------------------------------------
  out.push('## 4. Endings by route family');
  out.push('');
  out.push(
    table(
      ['Route family', 'Endings', 'Share of completed', 'Mean ending age', 'Median ending age'],
  ...
  // --- Fallback -------------------------------------------------------------
  out.push('## 5. Fallback by age band, conditional on the run still being active');
  out.push('');
  out.push(
    'Generic quiet-year fallback and the Phase-1.3 lore-fallback tier are kept separate. `Combined` is the ' +
      'substitution view — how often a year was filled by *some* fallback — and does not claim the generic ' +
      'problem is solved.',
  );
  out.push('');
  out.push(
    table(
      ...
      Object.entries(m.loreFallbackShareByAgeBandActive).map(([band, counts]) => [
        band,
        String(counts.activeYears),
        String(counts.genericFallbackYears),
        String(counts.loreFallbackYears),
        pct(counts.genericShare),
        pct(counts.loreShare),
        pct(counts.combinedShare),
      ]),
    ),
  );
  out.push('');
  out.push(
    `Unconditional age-25+ generic fallback share **${pct(m.fallbackShareAge25Plus)}** against a target of ` +
      `${pct(content.balance.fallbackGuardrails.targetFallbackShareAge25Plus, 0)}. ` +
      `Lore fallback is **${pct(m.loreFallbackShare, 2)}** of all event-years.`,
  );
  out.push('');

  // --- Schedules, FIX, material --------------------------------------------
  out.push('## 6. Schedules, FIX and material');
  out.push('');
  out.push(
    table(
      ['Metric', 'Value'],
      [
        ['Route entry rate', pct(m.routeEntryRate)],
        ['Route climax rate', pct(m.routeClimaxRate)],
        ['Route abandonment/expiry rate', pct(m.routeAbandonmentRate)],
        ['Expired schedules total', String(m.expiredScheduleCount)],
        ['Priority collision rate', pct(m.priorityCollisionRate)],
        ['Contested years total', String(m.priorityCollisionYears)],
        ['Schedule displacements per run', num(m.scheduleDisplacementPerRun, 2)],
        ['SPC share of event-years', pct(m.spcShare, 2)],
        ['Mean FIX at Material Commitment', num(m.meanFixAtCommitment, 2)],
        ['Mean FIX at ending', num(m.meanFixAtEnding, 2)],
        [
          'Final FIX p50 / p90 / p99 / max',
          `${num(m.finalFixPercentiles.p50, 0)} / ${num(m.finalFixPercentiles.p90, 0)} / ${num(m.finalFixPercentiles.p99, 0)} / ${num(m.finalFixPercentiles.max, 0)}`,
        ],
        ['Material Commitment rate', pct(m.materialCommitmentRate)],
        ['Average commitment age', num(m.averageCommitmentAge)],
      ],
  ...
  out.push('');
  out.push('Schedule expiry by route family:');
  out.push('');
  out.push(
    table(
      ['Route family', 'Expired schedules'],
      sortedEntries(m.scheduleExpiryByRouteFamily).map(([family, count]) => [`\`${family}\``, String(count)]),
    ),
  );
  out.push('');

  out.push('## 7. Balance warnings');
  out.push('');
  if (m.guardrails.length === 0) {
    out.push('_No guardrail findings._');
  } else {
